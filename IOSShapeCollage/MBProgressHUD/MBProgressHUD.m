...
//    NSLog(@"width - %.f", self.bounds.size.width);
    ...
    CGSize descStrSize = [labelText sizeWithAttributes:@{NSFontAttributeName:label.font}];
//    NSLog(@"descStrSize.width - %.f", descStrSize.width);
    
    
    while (descStrSize.width >= (label.frame.size.width * 0.75)) {
        fontSize -= 0.2;
        label.font = [UIFont boldSystemFontOfSize:fontSize];
        descStrSize = [labelText sizeWithAttributes:@{NSFontAttributeName:label.font}];
    }
...
//    NSLog(@"self - %@", self);
//    NSLog(@"keyWindow.subviews - %@", [UIApplication sharedApplication].keyWindow.subviews);
}
...
- (void)layoutSubviews {
    ...
    CGSize descStrSize = [label.text sizeWithAttributes:@{NSFontAttributeName:label.font}];
    
	descStrSize.width = MIN(descStrSize.width, maxWidth);
	totalSize.width = MAX(totalSize.width, descStrSize.width);
	totalSize.height += descStrSize.height;
	if (descStrSize.height > 0.f && indicatorF.size.height > 0.f) {
    ...
    CGRect textRect = [detailsLabel.text boundingRectWithSize:maxSize options:(NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading) attributes:@{NSFontAttributeName:detailsLabel.font} context:nil];
    CGSize detailsLabelSize = textRect.size;
    
    totalSize.width = MAX(totalSize.width, detailsLabelSize.width);
	totalSize.height += detailsLabelSize.height;
	if (detailsLabelSize.height > 0.f && (indicatorF.size.height > 0.f || descStrSize.height > 0.f)) {
		totalSize.height += kPadding;
	}
	...
	labelF.origin.x = roundf((bounds.size.width - descStrSize.width) / 2) + xPos;
	labelF.size = descStrSize;
	label.frame = labelF;
